...
function checkPassword() {
    ...
    var count1 = 0; //checking number of spCharacters in password
    var spChar = '!@#$%^&*()_';
    for (i = 0; i < password.length; i++) {
        for (j = 0; j < spChar.length; j++)
    ...
    for (i = 0; i < password.length; i++) { //checking number of numbers in password
    ...
    // console.log(count1)
    //console.log(count2)
    if (alphanumeric.test(password) && password.length > 5 && count2 > 0 && count1 > 0 && count > 0) {
        //console.log(true)
        return (true)
    } else {
        alert("You have entered an invalid Password!, password must contain atleast one Uppercase, one number and special characters")
        //console.log(false)
        return (false)
    }
